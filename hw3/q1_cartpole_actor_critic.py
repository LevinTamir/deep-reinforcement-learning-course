...
class Actor(nn.Module):
    """policy network π(a|s; θ) - Fixed 6 input, 3 output for meta-learning"""
    def __init__(self, obs_dim: int = 6, act_dim: int = 3, hidden: int = 256):
        super().__init__()
        # Use fixed sizes for meta-learning compatibility
        self.obs_dim = 6
        self.act_dim = 3
        self.net = nn.Sequential(
            ...
            nn.Tanh(),
            layer_init(nn.Linear(hidden, self.act_dim), std=0.01))
...
class Critic(nn.Module):
    """value function V(s; w) - Fixed 6 input for meta-learning"""
    def __init__(self, obs_dim: int = 6, hidden: int = 256):
        super().__init__()
        # Use fixed size for meta-learning compatibility
        self.obs_dim = 6
        self.net = nn.Sequential(
            layer_init(nn.Linear(self.obs_dim, hidden)),
            nn.Tanh(),
